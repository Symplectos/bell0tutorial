...
namespace graphics
{
	...
	D2D1_RECT_F Sprite::getCenteredRectangle(const float scaleFactor) const
	{
		...
		float h = (float)d2d.getVirtualHeight();
		float w = (float)d2d.getVirtualWidth();

		float centerWidth = (float)w / 2.0f;
		float centerHeight = (float)h / 2.0f;
	...
	void AnimatedSprite::drawCentered(const float scaleFactor, const float offsetX, const float offsetY, D2D1_RECT_F* const rect) const
	{
		...
		// get virtual screen
		float virtualWidth = (float)d2d.getVirtualWidth();
		float virtualHeight = (float)d2d.getVirtualHeight();
