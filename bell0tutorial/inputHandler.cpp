...
namespace input
{
	...
	// Keyboard and Mouse
	KeyboardAndMouse::KeyboardAndMouse() : mouseX(0), mouseY(0), scaleFactor(0.0f)
	{
	...
	Gamepad::Gamepad(const unsigned int playerID) : playerID(playerID)
	{
		...
		thumbStickLeft = new mathematics::linearAlgebra::Vector2F();
		thumbStickRight = new mathematics::linearAlgebra::Vector2F();
	}
	Gamepad::~Gamepad()
	{
	...
	InputHandler::InputHandler(core::DirectXApp& dxApp, const HINSTANCE& hInstance, const HWND& appWindow, const std::wstring& keyBindingsFileKeyboard, const std::wstring& keyBindingsFileJoystick, const std::wstring& keyBindingsFileGamepad) : keyBindingsFileKeyboard(keyBindingsFileKeyboard), keyBindingsFileJoystick(keyBindingsFileJoystick), keyBindingsFileGamepad(keyBindingsFileGamepad), dxApp(dxApp), listen(false), nGamepads(0), nPlayers(1)
	{
		...
		if (!readConfigFile().wasSuccessful())
			throw std::runtime_error("Unable to create the input handler, because the lua file couldn't be read!");

		// initialize keyboard and mouse
		kbm = new KeyboardAndMouse();
		kbm->scaleFactor = dxApp.getGraphicsComponent().getBiggestScalingFactor();
		activeMouse = true;
		activeKeyboard = true;
	...
	///////////////////////////// Keyboard and Mouse ////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////
	bool InputHandler::getTextInput()
	{
		kbm->textInput.clear();
		kbm->textInput.str(std::wstring());

		if (getKeyState(VK_BACK) == KeyState::JustPressed)
		{
			// send a depesche that the user desires to delete a letter or number
			std::deque<core::GameState*> states;
			dxApp.getActiveStates(states);
			for (std::deque<core::GameState*>::reverse_iterator it = states.rbegin(); it != states.rend(); it++)
			{
				...
				{
					core::DepescheDestination* destination = (*it);
					core::Depesche depesche(*this, *destination, core::DepescheTypes::TextDelete, nullptr);
					dxApp.addMessage(depesche);
				}
			}
		}

		if (getKeyState(VK_RETURN) != KeyState::JustPressed)
		{
			for (unsigned int i = 48; i <= 57; i++)
				if (getKeyState(i) == KeyState::JustPressed)
					kbm->textInput << getKeyName(i).get();

			for (unsigned int i = 65; i <= 90; i++)
				if (getKeyState(i) == KeyState::JustPressed)
					if (getKeyState(VK_SHIFT) == KeyState::JustPressed || getKeyState(VK_SHIFT) == KeyState::StillPressed)
						kbm->textInput << getKeyName(i).get();
					else
					{
						switch (i)
						{
						case 65: kbm->textInput << L"a"; break;
						case 66: kbm->textInput << L"b"; break;
						case 67: kbm->textInput << L"c"; break;
						case 68: kbm->textInput << L"d"; break;
						case 69: kbm->textInput << L"e"; break;
						case 70: kbm->textInput << L"f"; break;
						case 71: kbm->textInput << L"g"; break;
						case 72: kbm->textInput << L"h"; break;
						case 73: kbm->textInput << L"i"; break;
						case 74: kbm->textInput << L"j"; break;
						case 75: kbm->textInput << L"k"; break;
						case 76: kbm->textInput << L"l"; break;
						case 77: kbm->textInput << L"m"; break;
						case 78: kbm->textInput << L"n"; break;
						case 79: kbm->textInput << L"o"; break;
						case 80: kbm->textInput << L"p"; break;
						case 81: kbm->textInput << L"q"; break;
						case 82: kbm->textInput << L"r"; break;
						case 83: kbm->textInput << L"s"; break;
						case 84: kbm->textInput << L"t"; break;
						case 85: kbm->textInput << L"u"; break;
						case 86: kbm->textInput << L"v"; break;
						case 87: kbm->textInput << L"w"; break;
						case 88: kbm->textInput << L"x"; break;
						case 89: kbm->textInput << L"y"; break;
						case 90: kbm->textInput << L"z"; break;
						}
					}

			if (kbm->textInput.str().size() != 0)
			{
				// send a depesche that a letter or number was pressed
				std::deque<core::GameState*> states;
				dxApp.getActiveStates(states);
				for (std::deque<core::GameState*>::reverse_iterator it = states.rbegin(); it != states.rend(); it++)
				{
					...
					{
						core::DepescheDestination* destination = (*it);
						core::Depesche depesche(*this, *destination, core::DepescheTypes::TextInput, &kbm->textInput);
						dxApp.addMessage(depesche);
					}
				}
			}
	...
	void InputHandler::drawMouseCursor() const
	{
		if(activeMouse)
			if(kbm->mouseCursor != nullptr)
				kbm->mouseCursor->draw(kbm->scaleFactor);
	}
	...
	void InputHandler::getTransformedMousePosition(float& mouseX, float& mouseY) const
	{
		...
		mousePos.x = (float)kbm->mouseX;
		mousePos.y = (float)kbm->mouseY;
	...
	util::Expected<void> InputHandler::initializeDirectInput(const HINSTANCE& hInstance, const HWND& appWindow)
	{
		...
		result = initializeGameController(appWindow);
		if(!result.isValid())
	...
	////////////////////////////////// Configuration File ///////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////
	util::Expected<void> InputHandler::readConfigFile()
	{
		if (dxApp.getFileSystemComponent().hasValidConfigurationFile())
		{
			...
			{
				sol::state lua;
				if (!lua.script_file(util::StringConverter::ws2s(pathToPrefFile)).valid())
					return std::runtime_error("Unable to open lua file!");
