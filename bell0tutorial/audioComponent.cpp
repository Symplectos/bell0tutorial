...
namespace audio
{
	...
	AudioComponent::AudioComponent(const core::DirectXApp& dxApp) : dxApp(dxApp), streamingThread(nullptr)
	{
		...
		// set volume level
		if (!loadVolume().isValid())
			throw std::runtime_error("Failed to load volume level!");
	...
	/////////////////////////////////// Stream //////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////
	util::Expected<void> AudioComponent::streamFile(const std::wstring fileName, const AudioTypes type, const bool loop, const float frequency)
	{ 
		engine->stopStreaming = false;
		if(type == AudioTypes::Music)
			streamingThread = new std::thread(&AudioEngine::streamFile, engine, fileName, musicSendList, loop, frequency);
		else if(type == AudioTypes::Sound)
			streamingThread = new std::thread(&AudioEngine::streamFile, engine, fileName, soundsSendList, loop, frequency);
	...
	util::Expected<void> AudioComponent::onMessage(const core::Depesche& depesche)
	{
		...
		else if (depesche.type == core::DepescheTypes::BeginStream)
		{
			...
			util::Expected<void> result = streamFile(((StreamEvent*)depesche.message)->filename, ((StreamEvent*)depesche.message)->type, ((StreamEvent*)depesche.message)->loop, ((StreamEvent*)depesche.message)->frequency);
			if (!result.isValid())
				return result;
		}
		...
		if (dxApp.getFileSystemComponent().hasValidConfigurationFile())
		{
			...
			try
			{
				...
				if (!lua.script_file(util::StringConverter::ws2s(pathToPrefFile)).valid())
					return std::runtime_error("Unable to open lua script file!");
