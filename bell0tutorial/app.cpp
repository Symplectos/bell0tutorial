...
namespace core
{
	...
	////////////////////////////// Constructors /////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////
	DirectXApp::DirectXApp() : applicationIsPaused(true), fps(0), mspf(0.0), dt(1.0f/10000.0f), maxSkipFrames(100), applicationStarted(false), showFPS(true), stateStackChanged(false), audioComponent(nullptr), coreComponent(nullptr), fileSystemComponent(nullptr), graphicsComponent(nullptr), inputComponent(nullptr), numberTheory(nullptr) { }
	...
	util::Expected<void> DirectXApp::init(const HINSTANCE& hInstance, LPCWSTR windowTitle, const std::wstring& manufacturerName, const std::wstring& applicationName, const std::wstring& applicationVersion)
	{
		...
		// initialize the number theory component
		try { numberTheory = &mathematics::numberTheory::NumberTheory::getInstance(); }
		catch (std::runtime_error& e) { return e; }
		...
		if (!coreComponent->timer->start().wasSuccessful())
			return std::runtime_error("Unable to start timer!");
	...
	void DirectXApp::shutdown(const util::Expected<void>* /*expected*/)
	{
		...
		// the number theory component deletes itself
		/*if (numberTheory)
			delete numberTheory;*/
	...
	////////////////////////////// Quit Game ////////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////
	void DirectXApp::quitGame() const
	{
		PostMessage(coreComponent->appWindow->getMainWindowHandle(), WM_CLOSE, 0, 0);
	}
	...
	util::Expected<void> DirectXApp::calculateFrameStatistics()
	{
		...
		if ((coreComponent->timer->getTotalTime() - elapsedTime) >= 1.0)
		{
			...
			if (showFPS)
			{
				...
				outFPS << "Mode #" << graphicsComponent->getCurrentModeIndex() + 1 << " of " << graphicsComponent->getNumberOfSupportedModes() << std::endl;
	...
	/////////////////////////// Pause and Resume ////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////
	util::Expected<void> DirectXApp::pauseApplication()
	{
		applicationIsPaused = true;
		util::Expected<void> result = coreComponent->timer->stop();
		if (!result.isValid())
			return result;
	...
	util::Expected<void> DirectXApp::resumeApplication(bool recreateGraphics, bool restartTimer)
	{
		util::Expected<void> result;

		if (recreateGraphics && applicationStarted)
		{
			result = graphicsComponent->onResize(*this);
		...
		if (restartTimer)
		{
			result = coreComponent->timer->start();
			if (!result.isValid())
				return result;
			applicationIsPaused = false;
		}
	...
	util::Expected<void> DirectXApp::onNotify(const int event)
	{
		...
		case input::Events::PauseApplication:
			result = pauseApplication();
			if (!result.isValid())
				return result;
			break;

		case input::Events::ResumeApplication:
			result = resumeApplication(false, true);
			if (!result.isValid())
				return result;
			break;

		case input::Events::ChangeResolution:
			result = resumeApplication(true, false);
			if (!result.isValid())
				return result;
			break;
		...
		case input::Events::WindowChanged:
			result = resumeApplication(true, true);
			if (!result.isValid())
				return result;
			break;
	...
	mathematics::numberTheory::NumberTheory& DirectXApp::getNumberTheoryComponent() const
	{
		return *numberTheory;
	}
