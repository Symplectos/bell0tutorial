...
namespace input
{
	...
	private:
		...
		float scaleFactor;									// scaling factor for the mouse cursor (resolution independent rendering)

		std::wostringstream textInput;						// used when reading text
	public:
		KeyboardAndMouse();
	...
	private:
		...
		mathematics::linearAlgebra::Vector2F* thumbStickLeft;	// left thumb stick vector
		mathematics::linearAlgebra::Vector2F* thumbStickRight;	// right thumb stick vector
	...
	private:
		...
		bool listenForText;											// listen for text input
	...
	protected:
		...
		unsigned int offset = 293;												// offset for gamepad keys
		...
		util::Expected<void> readConfigFile();
	...
	public:
		...
		// get text input
		void enableListeningForText() { listenForText = true; };
		void disableListeningForText() { listenForText = false; };
		bool isListeningForTextInput() { return listenForText; };
		bool getTextInput();
		...
		void getTransformedMousePosition(float& mouseX, float& mouseY) const;
