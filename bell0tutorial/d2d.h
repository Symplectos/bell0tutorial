...
namespace graphics
{
	...
	private:
		...
		// brushes
		std::unordered_map<std::wstring, Microsoft::WRL::ComPtr<ID2D1SolidColorBrush> > brushes;	// hash table of all available brushes
	...
	public:
		...
		const unsigned int getVirtualWidth() const;
		const unsigned int getVirtualHeight() const;
		...
		ID2D1DeviceContext6& getDevCon() { return *devCon.Get(); };
		ID2D1Factory7& getFactory() { return *factory.Get(); };
	};
}